...
class Var(object):
    ...
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', "-1002235992780"))
    PORT = int(getenv('PORT', 8080))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', 'piro-streem.onrender.com')) ###### HEREEEEEEE #####
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = set(int(x) for x in os.environ.get("OWNER_ID", "1991522624").split())  
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = str(getenv('APP_NAME'))
    OWNER_USERNAME = str(getenv('OWNER_USERNAME', "SANDIP_01507"))
    if 'DYNO' in environ:
        ON_HEROKU = True
        APP_NAME = str(getenv('APP_NAME'))
    ...
    DATABASE_URL = str(getenv('DATABASE_URL', "mongodb+srv://thh123:thh123@cluster0.afbus.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"))
